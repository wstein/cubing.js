...
// src/cubing/twisty/model/props/puzzle/display/ExperimentalCubeColorSchemeProp.ts
var CUBE_FACES = ["U", "L", "F", "R", "B", "D"];
var BOY_CUBE_COLOR_SCHEME = Object.freeze({
  U: 16777215,
  L: 16744448,
  F: 4517376,
  R: 16711680,
...
var JAPANESE_CUBE_COLOR_SCHEME = Object.freeze({
  ...BOY_CUBE_COLOR_SCHEME,
  B: BOY_CUBE_COLOR_SCHEME.D,
  D: BOY_CUBE_COLOR_SCHEME.B
});
function isCubeFace(face) {
  return CUBE_FACES.includes(face);
}
function parseCustomCubeColorScheme(input) {
  ...
  for (const [rawFace, color] of Object.entries(input)) {
    ...
    if (typeof normalizedColor !== "string" && typeof normalizedColor !== "number" || normalizedColor === "" || typeof normalizedColor === "number" && !Number.isFinite(normalizedColor)) {
...
function splitSerializedEntries(input) {
  const entries = [];
  let depth = 0;
  let start = 0;
...
function parseSerializedCubeColorScheme(input) {
  ...
  for (const rawEntry of splitSerializedEntries(trimmed)) {
    ...
    scheme[entry.slice(0, separator).trim()] = entry.slice(separator + 1).trim();
  }
  return parseCustomCubeColorScheme(scheme);
}
function resolveCubeColorScheme(input) {
  if (input === void 0 || input === "default") {
    return void 0;
  }
  switch (input) {
    case "boy":
  ...
  return {
    ...BOY_CUBE_COLOR_SCHEME,
    ...typeof input === "string" ? parseSerializedCubeColorScheme(input) : parseCustomCubeColorScheme(input)
  };
}
...
var bulk3DCode = new LazyPromise(
  () => import("./twisty-dynamic-3d-PJNHN3YX.js")
);
...
//# sourceMappingURL=chunk-TWHZ6OH2.js.map
