...
var Cube3D = class extends Object3D {
  constructor(kpuzzle, scheduleRenderCallback, options = {}) {
    ...
    if (options.experimentalCubeColors !== void 0) {
  ...
  customStickerMaterials = /* @__PURE__ */ new Map();
  stickerMaterial(faceIdx, isHint, stickeringMask) {
    const defaultMaterial = isHint ? axesInfo[faceIdx].hintStickerMaterial[stickeringMask] : axesInfo[faceIdx].stickerMaterial[stickeringMask];
    const color = this.options.experimentalCubeColorScheme?.[CUBE_FACES[faceIdx]];
    if (typeof color === "undefined" || stickeringMask !== "regular" && stickeringMask !== "dim") {
    ...
    const materialColor = stickeringMask === "dim" ? regularColor === 16777215 ? 14540253 : new Color(regularColor).multiplyScalar(0.5).getHex() : regularColor;
        ...
        if (pieceStickeringMask) {
          ...
          for (let faceletIdx = 0; faceletIdx < pieceInfo.length; faceletIdx++) {
            ...
            if (faceletStickeringMask) {
              ...
              faceletInfo.stickeringMask = stickeringMask2;
  ...
  experimentalUpdateCubeColorScheme(scheme) {
    ...
    this.customStickerMaterials = /* @__PURE__ */ new Map();
...
var StickerDef = class {
  ...
  faceNum;
  faceName;
  constructor(filler, stickerDat, faceName, trim, options) {
    this.isDup = !!stickerDat.isDup;
    this.faceNum = stickerDat.face;
    this.faceName = faceName;
    this.stickerStart = filler.ipos;
    this.defaultColor = new Color2(stickerDat.color).getHex();
  ...
  cubeColor(scheme) {
    const color = this.faceName in (scheme ?? {}) ? scheme?.[this.faceName] : void 0;
    return new Color2(color ?? this.defaultColor).getHex();
  }
  ...
  setCubeColorScheme(filler, scheme) {
...
var PG3D = class extends Object3D2 {
  constructor(scheduleRenderCallback, kpuzzle, stickerDat, enableFoundationOpt = false, enableHintStickersOpt = false, hintStickerHeightScale = 1, faceletScale = 1, params = {}) {
    ...
    this.params = params;
    if (params.experimentalCubeColors !== void 0) {
      params.experimentalCubeColorScheme = params.experimentalCubeColors;
    }
    this.#isCubeShaped = stickerDat.faces.length === CUBE_FACES.length && CUBE_FACES.every(
      (face2) => stickerDat.faces.some(({ name }) => name === face2)
    );
    if (!this.#isCubeShaped) {
      params.experimentalCubeColorScheme = void 0;
    }
    ...
    for (const sticker of stickers) {
      ...
      if (params.experimentalCubeColorScheme) {
        options.experimentalCubeColorScheme = params.experimentalCubeColorScheme;
      }
  ...
  onPositionChange(p) {
    ...
    if (!this.lastPattern || this.#pendingStickeringUpdate || !this.lastPattern.isIdentical(pattern)) {
  ...
  experimentalUpdateCubeColorScheme(scheme) {
    if (!this.#isCubeShaped || this.params.experimentalCubeColorScheme === scheme) {
      return;
    }
...
async function pg3dShim(renderCallback, puzzleLoader, hintFacelets, faceletScale, darkIgnoredOrbits, experimentalCubeColorScheme) {
  return new PG3D(
    renderCallback,
...
//# sourceMappingURL=twisty-dynamic-3d-PJNHN3YX.js.map
